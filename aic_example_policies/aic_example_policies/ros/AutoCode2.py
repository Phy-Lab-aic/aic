...
class AutoCode2(Policy):
    """T3-specialized AutoCode fork (SC tilted-port focus, .autocode2 workspace).

    Forked from AutoCode.py at the AutoCode commit that scored
    min(trial_total) >= 95.67 on benchmark_06 (T1=95.80, T2=95.65, T3=95.69).
    This file is the only edit target for .autocode2 experiments;
    T1/T2 regressions are explicitly tolerated. AutoCode.py stays untouched
    as the leaderboard policy.
    """
    ...
    def insert_cable(
        ...
        self.get_logger().info(f"AutoCode2.insert_cable() task: {task}")
        self._task = task
        ...
        self.get_logger().info("AutoCode2.insert_cable() exiting...")
        return True
