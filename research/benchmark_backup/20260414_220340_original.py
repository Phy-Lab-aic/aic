...
# ── Approach ──────────────────────────────────────────────────────────
APPROACH_Z_OFFSET = -0.08         # port z축 방향 접근 offset (m, 음수=삽입 반대방향)
APPROACH_VELOCITY = 0.05          # approach 이동 속도 (m/s)
APPROACH_MIN_STEPS = 75           # 최소 approach 스텝 (가까워도 최소 1초)
DT = 0.05                         # 제어 주기 (초, 20Hz)

# ── Descent + Insertion ───────────────────────────────────────────────
DESCENT_STEP = 0.0005             # z_offset 증가량 (m/step, 삽입 방향으로 0.5mm)
DESCENT_STEP_MIN = 0.0001         # force-adaptive 최소 step (0.1mm)
INSERT_Z_LIMIT = 0.02             # 최대 삽입 깊이 z_offset (m, port z축 양수 = 삽입 진행)

# ── Stiffness ─────────────────────────────────────────────────────────
STIFFNESS_DEFAULT = [90.0, 90.0, 90.0, 50.0, 50.0, 50.0]
...
# ── Stabilize ─────────────────────────────────────────────────────────
STABILIZE_DURATION = 1.0          # 최종 안정화 (초)
...
class OptimalPolicy(Policy):
    ...
    def insert_cable(self, task: Task, get_observation: GetObservationCallback,
        ...
        self._log(f"  Phase 3: descent from z_offset={z_offset}m, "
                  f"step={DESCENT_STEP}m, limit={INSERT_Z_LIMIT}m")
        ...
        while z_offset < INSERT_Z_LIMIT:
            ...
            # ── Force-adaptive descent step ──
            if f_abs > 5.0:
                step = max(DESCENT_STEP_MIN,
                           DESCENT_STEP * (1.0 - f_abs / 20.0))
            else:
                step = DESCENT_STEP
            ...
            # ── Stiffness 전환 (port에 가까워지면 insertion stiffness) ──
            if z_offset > STIFFNESS_TRANSITION_Z:  # -0.02m 이상이면 port 근접
                stiffness = STIFFNESS_INSERT
            else:
            ...
            # ── 로깅 (매 20스텝) ──
            if descent_step_count % 20 == 0:
                ds = f"{depth:.5f}" if depth is not None else "N/A"
                self._log(f"  P3[{descent_step_count:4d}] z_off={z_offset:.4f}m "
