...
- `pixi.toml` 추가: venv + npm 수동 설치 대신 pixi 기반으로 통합
- `start_pixi.sh` 추가: pixi 환경용 실행 스크립트
- `hf-auto-mount.service`: ExecStart 경로를 로컬 환경에 맞게 수정

## Quick Start (pixi)

```bash
cd custom_tools/curation-tools
pixi install && pixi run setup   # 최초 1회
pixi run start                    # 실행 → http://localhost:5173
```
